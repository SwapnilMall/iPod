...
class App extends React.Component {
	...
	leftButtonClicked = () => {
		...
		if (
			...
			document.getElementsByClassName('screen-menu')[0].classList.contains('width-50') //if the menu is open and it is on the songs page only then if the left button clicked, menu will be changed to general options
		)
			this.setState({
				options: this.state.general_menu,
				song_index: -1,
		...
		if (!document.getElementsByClassName('screen-menu')[0].classList.contains('width-50')) {
			...
			if (this.state.options.length === 3) {
				...
				if (this.state.showPage === 0) {
					//I am on all songs page
					if (
						this.state.current_music_selection === 0 //If I am playing the music at 5th index then I will need to reduce the index to 0 on next right button click.
					)
						this.setState({
		...
		if (!document.getElementsByClassName('screen-menu')[0].classList.contains('width-50')) {
			...
			if (this.state.options.length === 3) {
				...
				if (this.state.showPage === 0) {
					//I am on all songs page
					if (
						this.state.current_music_selection === 5 //If I am playing the music at 5th index then I will need to reduce the index to 0 on next right button click.
					)
						this.setState({
							current_music_selection: 0
						});
